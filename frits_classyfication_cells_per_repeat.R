...
alp_model_f.sso<-merge(alp_model_f.ss[,c(1:3,38)],rslt,by="ImageNumber") 
...
alp_model_f.sso3<-merge(alp_model_f.sso2,unique(alp_model_f.sso[,c(1,4)]),by="ImageNumber",
                        all.y=F)

alp_model_f.sss<-alp_model_f.sso3[,-c(1:4)]
##creating model
...
feature_class<-unique(alp_model_f.sso3[,c(2,38)])
class_data_feat<-feature_class[,"Class"]
FeatureInTrain<-createDataPartition(class_data_feat, p =3/4, list = FALSE)
inTrain<-alp_model_f.sso3$feature.idx%in%feature_class[FeatureInTrain,"feature.idx"]
...
save(list=ls(),file="classyfication cell per repeat.RDATA")
