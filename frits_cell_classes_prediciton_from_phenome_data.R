...
alp_model<-featind
##loading phenome data
load("data/non_correlated_surfaces.RData")

#attach alp rank to morphological data
alp_model_morph.temp<-merge(alp_model,non_cor_feat_data,by.x="feature.idx",by.y="FeatureIdx")
#Selecting features for anlaysis
alp_model_morph<-alp_model_morph.temp[,c("feature.idx",
...
# library(plyr)
# alp_model_f.sso2<-ddply(alp_model_f.ss,"ImageNumber", numcolwise(median))
...
# 
# alp_model_f.sss.t<-alp_model_f.sso2.f[,-c(2,3)]
# rownames(alp_model_f.sss.t)<-alp_model_f.sss.t$feature.idx
alp_model_f.ss
alp_model_f.sss<-alp_model_f.ss[,-1]
...
alp_model_f.sss.scale<-alp_model_f.sss
...
to.dist.cl<-alp_model_f.sss.pca$x[,1:3]
...
plot(hclustres)
clust.numb<-5 ##specify number of clusters for selection
