...
alp_model_f.sss<-alp_model_f.sso[,-c(1:3,5)]
##creating model
...
feature_class<-unique(alp_model_f.sso[,c(2,4)])
class_data_feat<-feature_class[,"Class"]
FeatureInTrain<-createDataPartition(class_data_feat, p =3/4, list = FALSE)
inTrain<-alp_model_f.sso$feature.idx%in%feature_class[FeatureInTrain,"feature.idx"]
...
save(list=ls(),file="classyfication cell per cell.RDATA")
